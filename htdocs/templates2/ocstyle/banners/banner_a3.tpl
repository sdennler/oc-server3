...
<div id="oc-banner-a" role="banner" aria-label="Spendenaufruf opencaching.de">
    <style>
        ...
        @media (prefers-reduced-motion: reduce) {
            #oc-banner-a.oc-banner-dismissing { transition: none; }
        }

        .oc-banner-a__inner {
            display: flex; align-items: center; justify-content: center;
            gap: 12px; padding: 10px 48px 10px 16px; min-height: 44px; box-sizing: border-box;
        }
        .oc-banner-a__text { flex: 1 1 auto; text-align: center; line-height: 1.4; }
        .oc-banner-a__cta {
            flex: 0 0 auto; display: inline-block; padding: 6px 16px;
            background-color: var(--oc-banner-btn-bg); color: var(--oc-banner-btn-text);
            font-weight: bold; font-size: 13px; text-decoration: none;
            text-transform: uppercase; white-space: nowrap; line-height: 1.4;
        }
        .oc-banner-a__cta:hover, .oc-banner-a__cta:focus {
            background-color: #FF9999; color: var(--oc-banner-btn-text);
            text-decoration: none; outline: 2px solid #ffffff; outline-offset: 2px;
        }
        .oc-banner-a__close {
            position: absolute; top: 50%; right: 12px; transform: translateY(-50%);
            background: none; border: none; color: var(--oc-banner-close-color);
            font-size: 20px; line-height: 1; cursor: pointer; padding: 4px 6px;
        }
        .oc-banner-a__close:hover, .oc-banner-a__close:focus {
            color: #ffffff; background-color: rgba(255,255,255,0.15);
            outline: 2px solid #ffffff; outline-offset: 1px;
        }
        @media (max-width: 599px) {
            .oc-banner-a__inner { flex-direction: column; text-align: center; padding: 12px 44px 12px 12px; gap: 8px; }
            .oc-banner-a__text { font-size: 13px; }
        }
    </style>
    ...
    <span class="oc-banner-a__text">
      opencaching.de lebt von eurer Unterstützung. Bitte spendet!
        ...
        <a class="oc-banner-a__cta" id="oc-banner-a__cta-link" href="#"
            target="_blank" rel="noopener noreferrer"
            aria-label="Jetzt spenden — opencaching.de unterstützen">Jetzt spenden</a>
    </div>
    <button class="oc-banner-a__close" id="oc-banner-a__close-btn" type="button"
        aria-label="Banner schließen">&#x2715;</button>
</div>
    ...
    (function () {
        ...
        function isDismissed() {
            try {
                ...
                if (!raw) return false;
                return Date.now() < new Date(raw).getTime() + DISMISS_TIME;
        ...
        function dismissBanner(banner, restoreFocusTo) {
            ...
            if (prefersReduced) {
                banner.style.display = 'none'; removeBannerLayout(); setDismissed();
                if (restoreFocusTo && restoreFocusTo.focus) restoreFocusTo.focus(); return;
            }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            if (isDismissed()) {
                var b = document.getElementById(BANNER_ID); if (b) b.style.display = 'none';
                removeBannerLayout(); return;
            }
